...
import {
  ...
  ChevronLeft, ChevronRight, FileText, Camera, User, Truck,
} from 'lucide-react';
...
export default function AdminDeliveryPartnersPage() {
  ...
  const handleAction = async (partnerId, action, reason, documentType = null, vehicleId = null) => {
    try {
  ...
  const getStatusBadge = (partner) => {
    if (partner.verificationStatus === 'APPROVED' || partner.isVerified) {
      return (
        <span className="inline-flex items-center gap-1 px-2.5 py-1 text-xs rounded-full bg-green-100 text-green-700 font-semibold border border-green-200">
    ...
    if (partner.verificationStatus === 'REJECTED') {
      return (
        <span className="inline-flex items-center gap-1 px-2.5 py-1 text-xs rounded-full bg-red-100 text-red-700 font-semibold border border-red-200">
    ...
    if (partner.licenseDoc || partner.profilePhoto || (partner.vehicles && partner.vehicles.length > 0)) {
      return (
        <span className="inline-flex items-center gap-1 px-2.5 py-1 text-xs rounded-full bg-yellow-100 text-yellow-700 font-semibold border border-yellow-200">
          <Clock className="h-3 w-3" /> Pending Review
        </span>
      );
    }
    return (
      <span className="inline-flex items-center gap-1 px-2.5 py-1 text-xs rounded-full bg-gray-100 text-gray-600 font-semibold border border-gray-200">
        <AlertTriangle className="h-3 w-3" /> No Documents
      </span>
    );
  };
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Verify and manage delivery partner accounts • {stats.total || 0} total
          </p>
        </div>
        <button
          onClick={fetchPartners}
          className="px-4 py-2 bg-white border rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors"
        >
          ↻ Refresh
        </button>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search by name, mobile, vehicle number, license..."
        ...
        <select
          className="border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-orange-500 outline-none bg-white"
          value={filter}
                ...
                <tr>
                  <td colSpan={6} className="px-4 py-16 text-center">
                    ...
                    <p className="text-gray-400 text-sm mt-1">
                      {filter ? 'Try changing the filter' : 'Partners will appear here after registration'}
                    </p>
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={p.id} className="hover:bg-orange-50/30 transition-colors">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="relative">
                          {p.profilePhoto ? (
                            ...
                            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center text-white font-bold text-sm shadow-sm">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500 flex items-center gap-1">
                            <Phone className="h-3 w-3" /> {p.user?.mobile || 'N/A'}
                          </p>
                          {p.licenseNumber && (
                            <p className="text-xs text-gray-400 flex items-center gap-1 mt-0.5">
                              <Shield className="h-3 w-3" /> {p.licenseNumber}
                            </p>
                          )}
                        </div>
                      </div>
                    ...
                    <td className="px-4 py-3.5">
                      <p className="text-sm font-medium text-gray-900">
                        {p.activeVehicle?.vehicleType || 'Not set'}
                      </p>
                      <p className="text-xs text-gray-500 flex items-center gap-1">
                        <Bike className="h-3 w-3" /> {p.activeVehicle?.vehicleNumber || 'Not provided'}
                      </p>
                      {p.vehicles && p.vehicles.length > 1 && (
                        <p className="text-xs text-orange-500 mt-0.5">+{p.vehicles.length - 1} more</p>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex flex-wrap gap-1.5">
                        {p.licenseDoc && (
                          <button onClick={() => setPreviewImage({ url: p.licenseDoc, label: 'Driving License' })} className="text-xs bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded font-medium hover:bg-blue-100">DL</button>
                        )}
                        {p.activeVehicle?.rcDocument && (
                          <button onClick={() => setPreviewImage({ url: p.activeVehicle.rcDocument, label: 'Vehicle RC' })} className="text-xs bg-green-50 text-green-600 px-1.5 py-0.5 rounded font-medium hover:bg-green-100">RC</button>
                        )}
                        {p.profilePhoto && (
                          <button onClick={() => setPreviewImage({ url: p.profilePhoto, label: 'Profile Photo' })} className="text-xs bg-purple-50 text-purple-600 px-1.5 py-0.5 rounded font-medium hover:bg-purple-100">Photo</button>
                        )}
                        {p.selfieWithVehicle && (
                          <button onClick={() => setPreviewImage({ url: p.selfieWithVehicle, label: 'Selfie' })} className="text-xs bg-orange-50 text-orange-600 px-1.5 py-0.5 rounded font-medium hover:bg-orange-100">Selfie</button>
                        )}
                        {!p.licenseDoc && !p.activeVehicle?.rcDocument && !p.profilePhoto && !p.selfieWithVehicle && (
                    ...
                    <td className="px-4 py-3.5">
                      {getStatusBadge(p)}
                      {p.verificationNote && (
                        <p className="text-xs text-red-500 mt-1.5 max-w-[180px] truncate italic" title={p.verificationNote}>
                          "{p.verificationNote}"
                        </p>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-3.5">
                      <div className="space-y-1">
                        <p className="text-sm font-semibold text-gray-900">{p.totalDeliveries || 0} deliveries</p>
                      ...
                      <div className="flex items-center justify-center gap-1.5">
                        ...
                        {!p.isVerified && p.verificationStatus !== 'REJECTED' && (
                          <>
                            ...
                            <button
                              onClick={() => { setSelectedPartner(p); setShowRejectModal(true); }}
                        ...
                        {p.isVerified && (
                          <button
                            onClick={() => handleAction(p.id, 'unverify')}
        ...
        >
          ...
          >
            ...
            <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 rounded-t-2xl flex items-center justify-between z-10">
              <div>
                <h3 className="text-lg font-bold text-gray-900">Partner Details</h3>
                <p className="text-sm text-gray-500">Full verification review</p>
              </div>
              <button
                onClick={() => setSelectedPartner(null)}
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex items-center gap-4 p-5 bg-gradient-to-r from-orange-50 to-rose-50 rounded-2xl border border-orange-100">
                {selectedPartner.profilePhoto ? (
                  ...
                  <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center text-2xl font-bold text-white shadow-lg">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600 flex items-center gap-1">
                    <Phone className="h-3.5 w-3.5" /> {selectedPartner.user?.mobile || 'N/A'}
                  </p>
                  {selectedPartner.licenseNumber && (
                    <p className="text-xs text-gray-500 flex items-center gap-1 mt-0.5">
                      <Shield className="h-3 w-3" /> License: {selectedPartner.licenseNumber}
                    </p>
                  )}
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  ...
                  <div key={i} className={`${item.color} rounded-xl p-3.5`}>
                    <item.icon className="h-4 w-4 mb-1 opacity-70" />
              ...
              <div>
                <h4 className="text-sm font-semibold text-gray-900 mb-3 flex items-center gap-2">
                  <Camera className="h-4 w-4" /> Document Verification (Per-Document)
                </h4>
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    const statusBadge = docStatus === 'APPROVED' 
                    ...
                    const statusText = docStatus === 'APPROVED' ? '✓ Approved'
                    ...
                    return (
                      <div key={i} className="border border-gray-200 rounded-xl overflow-hidden">
                        {doc.url ? (
                          <button onClick={() => setPreviewImage({ url: doc.url, label: doc.label })} className="w-full">
                        ...
                        <div className="p-2.5 space-y-2">
                          ...
                          {docStatus === 'REJECTED' && docRejection && (
                            <p className="text-xs text-red-500 italic truncate" title={docRejection}>💬 {docRejection}</p>
                          )}
                          {doc.url && docStatus !== 'APPROVED' && (
                            <div className="flex gap-1.5 pt-1">
                              <button
                                onClick={() => handleAction(selectedPartner.id, 'approve_doc', null, doc.key)}
                                className="flex-1 px-2 py-1.5 text-xs bg-emerald-50 text-emerald-700 rounded-lg hover:bg-emerald-100 font-medium border border-emerald-200"
                              >✓ Approve</button>
                              <button
                                onClick={() => {
                                  setSelectedPartner({...selectedPartner, _rejectDoc: doc.key});
                                  setShowRejectModal(true);
                                }}
                                className="flex-1 px-2 py-1.5 text-xs bg-red-50 text-red-600 rounded-lg hover:bg-red-100 font-medium border border-red-200"
                              >✕ Reject</button>
                            </div>
                          )}
                          {docStatus === 'APPROVED' && (
                            <button
                              onClick={() => handleAction(selectedPartner.id, 'reject_doc', 'Document needs review', doc.key)}
                              className="w-full px-2 py-1.5 text-xs bg-red-50 text-red-600 rounded-lg hover:bg-red-100 font-medium border border-red-200"
                            >↩ Revoke</button>
                          )}
                        </div>
                      </div>
              ...
              {selectedPartner.vehicles && selectedPartner.vehicles.length > 0 && (
                <div>
                  <h4 className="text-sm font-semibold text-gray-900 mb-3 flex items-center gap-2">
                    <Truck className="h-4 w-4" /> Vehicles ({selectedPartner.vehicles.length})
                  </h4>
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="text-sm font-semibold">
                              {selectedPartner.activeVehicleId === v.id && '✅ '}
                              {v.vehicleType} — {v.vehicleNumber || 'No number'}
                            </p>
                            <p className="text-xs text-gray-500">
                              {v.isVerified ? '✓ Verified' : v.verificationStatus === 'REJECTED' ? '✕ Rejected' : '⏳ Pending'}
                              {v.verificationNote && <span className="text-red-500 ml-1">— {v.verificationNote}</span>}
                            </p>
                          </div>
                          <div className="flex gap-1">
                            {v.rcDocument && (
                              <button onClick={() => setPreviewImage({ url: v.rcDocument, label: `RC - ${v.vehicleNumber}` })} className="px-2 py-1 text-xs bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100">View RC</button>
                            )}
                            {!v.isVerified && v.verificationStatus !== 'REJECTED' && (
              ...
              <div className="flex gap-3 pt-4 border-t border-gray-200">
                {!selectedPartner.isVerified ? (
                  <button
                    onClick={() => handleAction(selectedPartner.id, 'verify')}
                  ...
                  <button
                    onClick={() => handleAction(selectedPartner.id, 'unverify')}
                    className="flex-1 bg-amber-600 text-white py-3 rounded-xl font-semibold hover:bg-amber-700 transition-colors flex items-center justify-center gap-2"
                  >
        ...
        >
          ...
          >
            <h3 className="text-lg font-bold text-gray-900 mb-1">
              {selectedPartner._rejectDoc ? 'Reject Document' : 'Reject Partner'}
            </h3>
            <p className="text-sm text-gray-500 mb-4">
              Select a reason — this will be shown to {selectedPartner.user?.name || 'the partner'}
            </p>
            ...
            <div className="space-y-2 max-h-[260px] overflow-y-auto mb-4 pr-1">
              {REJECT_REASONS.map((reason, i) => (
                <button
                  ...
                  className={`w-full text-left p-3 rounded-xl text-sm border-2 transition-all ${
                    rejectReason === reason
            ...
            <textarea
              className="w-full border-2 border-gray-200 rounded-xl p-3 text-sm mb-4 focus:border-red-500 focus:ring-1 focus:ring-red-500 outline-none resize-none"
              rows={2}
            ...
            <div className="flex gap-3">
              <button
                onClick={() => {
                  ...
                  if (docKey) {
                    handleAction(selectedPartner.id, 'reject_doc', rejectReason || 'Document needs attention', docKey);
                  } else {
                    handleAction(selectedPartner.id, 'reject', rejectReason || 'Documents need attention');
                  }
                }}
                disabled={!rejectReason.trim()}
                className="flex-1 bg-red-600 text-white py-3 rounded-xl font-semibold hover:bg-red-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2"
              >
                <XCircle className="h-4 w-4" /> {selectedPartner._rejectDoc ? 'Reject Document' : 'Reject Partner'}
              </button>
              <button
                onClick={() => { setShowRejectModal(false); setRejectReason(''); }}
                className="flex-1 bg-gray-100 text-gray-700 py-3 rounded-xl font-semibold hover:bg-gray-200 transition-colors"
